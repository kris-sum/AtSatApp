	...
	this.updateDeviceMove = function () {
		...
		return function () {
			...
			if (this.deviceOrientation.webkitCompassHeading) { 
				...
				if (iosAlphaAdjustment==360 && this.deviceOrientation.webkitCompassHeading > 0) {
					//take the current alpha value, add the difference to north.
					iosAlphaAdjustment = this.deviceOrientation.webkitCompassHeading;
				}
				var adjustedAlpha = (360-this.deviceOrientation.alpha) + iosAlphaAdjustment;
				if (adjustedAlpha > 360) { 
					adjustedAlpha -=360;
				}
			
			  alpha  = THREE.Math.degToRad( 360-adjustedAlpha); // Z
			} else { 
			...
			/*
			$('#debug').html('alpha = '+this.deviceOrientation.alpha+' ('+adjustedAlpha+') compass='+this.deviceOrientation.webkitCompassHeading  + 'adjustment = '+iosAlphaAdjustment);
			*/
